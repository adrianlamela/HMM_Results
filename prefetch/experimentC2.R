...
borrar <- foreach(i = 1:length(nombres),.export = c("nombres","longitudAnalisis")) %dopar% {
  ...
  nombre <- nombres[i]
  sink(paste("outputC2_",nombre,".out",sep=""))
  ...
  nombreSalida <- paste("resultadosC2/resultadosC2_",nombre,".RData",sep="")
  inicializarCache(2**24,asociatividad = 4)
  resultados <- prebusquedaRapida(data$tag,nombreSalida,ngroups = 10,maxH = 50,eager = 0.7,niter = 1)
  sink()
}
rm(borrar)
