...
borrar <- foreach(i = 1:length(nombres),.export = c("nombres","longitudAnalisis")) %dopar% {
  ...
  nombre <- nombres[i]
  sink(paste("outputC5_",nombre,".out",sep=""))
  ...
  nombreSalida <- paste("resultadosC5/resultadosC5_",nombre,".RData",sep="")
  inicializarCache(2**24,asociatividad = 4)
  resultados <- prebusquedaRapida(data$tag,nombreSalida,ngroups = 20,maxH = 20,eager = 0.9,niter = 1)
  sink()
}
rm(borrar)
