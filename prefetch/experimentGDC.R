...
borrar <- foreach(i = 1:length(nombres),.export = c("nombres","longitudAnalisis")) %dopar% {
  ...
  nombre <- nombres[i]
  sink(paste("outputGDC_",nombre,".out",sep=""))
  ...
  nombreSalida <- paste("resultadosGDC/resultadosGDC_",nombre,".RData",sep="")
  inicializarCache(2**24,asociatividad = 4)
  resultados <- prebusquedaGDC(data$tag,nombreSalida,neit=512,neghb=512,depth=4,width=4)
  sink()
}
rm(borrar)
